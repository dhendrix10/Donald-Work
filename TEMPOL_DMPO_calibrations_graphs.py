...
def get_da_dFs(filename):
    ...
    #Opens file
    """filename is an argument; y = f(x); y is returned value; x is"""
...
#plot = True is an argument that will plot, =False will never plot
def get_concentrations(path,plot=True):
    ...
    #Data dictionary
    #key is the file and value is a dictionary
    data = {}
    
    #Gets all the files in the concentrations folder
    """Copy and past files into "path" below"""
    ...
    if plot:
        ...
        for key, value in data.items():
            ...
            #try because this only work for TEMPOL, /10 because of the way I
            #saved the files
        
        #Sets the tick marks
        #adds tick mark at each number    
...
def tmp_cal(path,peaks,cal_plot=False,std_plot=False):
    ...
    if cal_plot:
        ...
        ax.plot([], [], '', label = f"r^2 = {regressor.score(x, y)}",
                c = 'white')
